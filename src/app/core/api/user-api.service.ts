...
export class UserApiService {
  ...
  getUsersAllPages(query?: Omit<UsersQueryParams, 'page' | 'limit'>): Observable<User[]> {
    ...
    const load = (page: number, acc: User[]): Observable<User[]> =>
      this.getUsersPage({ ...query, page, limit }).pipe(
        switchMap((payload) => {
  ...
  uploadAvatarForUser(
    userId: string,
    file: File,
    multipartFieldName: string = 'avatar',
  ): Observable<User> {
    const formData = new FormData();
    formData.append(multipartFieldName, file, file.name);
    return this.http
      .post<
        ApiResponse<User>
      >(`${this.baseUrl}/users/${encodeURIComponent(userId)}/avatar`, formData)
      .pipe(map((res) => res.data));
  }
  ...
  private parseUsersResponseBody(
    body: unknown,
    queryFallback?: UsersQueryParams,
  ): UsersPagePayload {
