...
import {
  ...
  UserRole,
  DEFAULT_USER_ROLE,
} from '@app/core/constants/roles.constants';
...
export class InviteSetupWizardDialogComponent implements OnInit {
  ...
  isBusy = computed(
    () => this.isDeptSubmitting() || this.isPosSubmitting() || this.isInviteSubmitting(),
  );
  stepCaption = computed(() => {
    switch (this.step()) {
      case 0:
        return 'invites.wizard.stepDepartment';
      case 1:
        return 'invites.wizard.stepPosition';
      default:
        return 'invites.wizard.stepInvitation';
    }
  });
  ngOnInit(): void {
    const allowed = this.invitableRoles();
    if (!allowed.length) {
      this.snackBar.open(this.t('messages.invites.roleCannotSend'), this.t('common.close'), { duration: 4000 });
    ...
    forkJoin({
      ...
      .subscribe({
        ...
        error: () => {
          this.snackBar.open(
            this.t('messages.invites.loadMetaFailed'),
            this.t('common.close'),
            {
              duration: 5000,
            },
    ...
    this.departmentApi
      ...
      .subscribe({
        next: (dept) => {
          const id = dept.id;
          if (!id) {
            this.snackBar.open(this.t('messages.invites.departmentNoId'), this.t('common.close'), {
              duration: 4000,
            });
            return;
          }
        ...
        error: (err: HttpErrorResponse) => {
          this.snackBar.open(this.describeError(err, this.t('messages.invites.departmentCreateFailed')), this.t('common.close'), {
            duration: 5000,
          });
        },
      });
    ...
    this.positionApi
      ...
      .subscribe({
        next: (pos) => {
          const id = pos.id;
          if (!id) {
            this.snackBar.open(this.t('messages.invites.positionNoId'), this.t('common.close'), { duration: 4000 });
            return;
          }
          this.createdPositionId.set(id);
        ...
        error: (err: HttpErrorResponse) => {
          this.snackBar.open(this.describeError(err, this.t('messages.invites.positionCreateFailed')), this.t('common.close'), {
            duration: 5000,
          });
        },
      });
  ...
  onSendInvite(): void {
    ...
    if (!deptId || !posId) {
      this.snackBar.open(this.t('messages.invites.missingDepartmentOrPosition'), this.t('common.close'), {
        duration: 4000,
      });
      return;
    }
    ...
    this.inviteService
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          this.snackBar.open(this.describeError(err, this.t('messages.invites.sendFailed')), this.t('common.close'), {
            duration: 6000,
          });
        },
      });
